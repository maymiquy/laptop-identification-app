...
const Edit = () => {
    const { auth, kerusakan, list, data_gejala } = usePage().props;
    ...
    const handleGejalaChange = (gejalaId) => {
        setData(
            "gejala",
            data.gejala.includes(gejalaId)
                ? data.gejala.filter((id) => id !== gejalaId)
                : [...data.gejala, gejalaId]
        );
    };

    const onSubmit = (e) => {
        e.preventDefault();
        patch(route("dashboard.kerusakan.update", kerusakan.kode_kerusakan), {
            onSuccess: () => reset(),
        });
    };
