...
const Index = () => {
    const { auth, kerusakan: dataKerusakan, flash } = usePage().props;
    const [currentPage, setCurrentPage] = React.useState(1);
    const [kerusakan, setKerusakan] = React.useState(dataKerusakan);
    const [showDeleteModal, setShowDeleteModal] = React.useState(false);
    const [showMessageModal, setShowMessageModal] = React.useState(false);
    const [deleteKerusakan, setDeleteKerusakan] = React.useState(null);

    const itemsPerPage = 10;

    const totalPages = Math.ceil(kerusakan.length / itemsPerPage);

    const indexOfLastItem = currentPage * itemsPerPage;
    const indexOfFirstItem = indexOfLastItem - itemsPerPage;
    const currentItems = kerusakan.slice(indexOfFirstItem, indexOfLastItem);

    const handlePageChange = (pageNumber) => {
        setCurrentPage(pageNumber);
    };

    const handleDeleteModal = (kerusakan) => {
        setDeleteKerusakan(kerusakan);
        setShowDeleteModal(true);
    };

    const confirmDelete = () => {
        router.delete(
            route("dashboard.kerusakan.destroy", {
                kerusakan: deleteKerusakan.kode_kerusakan,
            }),
            {
                preserveScroll: true,
                onSuccess: () => {
                    setKerusakan(
                        kerusakan.filter(
                            (k) =>
                                k.kode_kerusakan !==
                                deleteKerusakan.kode_kerusakan
                        )
                    );
                    ...
                    <div className="max-w-7xl flex-col flex justify-between mx-auto">
                        <div className="bg-white flex flex-col items-center shadow-md rounded-md md:rounded-lg pt-3 pb-6 px-6 md:px-8">
                            <div className="flex flex-col w-full gap-4 py-2 justify-start">
                                <h1 className="text-2xl">Data Kerusakan</h1>
                                <Link href="/dashboard/kerusakan/create">
                            ...
                            <div className="mt-3 sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div className="mt-2">
                                    <p className="text-sm text-gray-500">
                                        ...
                                        kerusakan{" "}
                                        {deleteKerusakan?.nama_kerusakan}?
                                    </p>
                                </div>
                            </div>
                        </div>
